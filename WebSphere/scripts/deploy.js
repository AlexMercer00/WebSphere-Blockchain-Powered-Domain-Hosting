...
async function main() {
  ...
  const WebSphere = await WebSphere.deploy(NAME, SYMBOL)
  await WebSphere.deployed();

  console.log(`Deployed Domain Contract at: ${WebSphere.address}\n`)
  ...
  for (var i = 0; i < 6; i++) {
    const transaction = await WebSphere.connect(deployer).list(names[i], costs[i])
